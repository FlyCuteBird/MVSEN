...
def encode_data(model, data_loader, log_step=10, logging=print):
    ...
    max_n_word = 0
    for i, (images, captions, lengths, labels, ids, po_captions, po_lengths) in enumerate(data_loader):
    ...
    for i, (images, captions, lengths, labels, ids, po_captions, po_lengths) in enumerate(data_loader):
        ...
        # compute the embeddings img_emb, cap_emb, cap_lens, po_cap_emb, po_cap_lens
        img_emb, cap_emb, cap_len, cap_emb_orignal, po_cap_emb, po_cap_lens = model.forward_emb(
            images, captions, lengths, po_captions, po_lengths, volatile=True)
...
def evalrank(model_path, data_path=None, split='dev', fold5=False):
    ...
    if not fold5:
        ...
        np.save(path + '%s_%s_%s_sim.npy' % (opt.data_name, opt.batch_size, opt.Matching_direction), sims)
        ...
        for i in range(5):
            ...
            np.save(path + '%s_%s_%s_%s_sim.npy' % (opt.data_name, opt.batch_size, opt.Matching_direction, i), sims)
            end = time.time()
            print("calculate similarity time:", end - start)
...
def shard_xattn(model, images, captions, caplens, opt, shard_size=64):
    ...
    for i in range(n_im_shard):
        ...
        for j in range(n_cap_shard):
            ...
            sim, _, _ = model.forward_sim(im, s, l)
